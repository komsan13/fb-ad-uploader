...
function apPrune(s) {
  for (const key of ['handled', 'retryOf', 'retries', 'counted', 'paused']) {
    const bag = s[key] || {};
...
function saveApMerged(s) {
  const cur = loadAp();
  s.killSwitch = cur.killSwitch;                       // ปุ่มหยุดฉุกเฉินเป็นของผู้ใช้เสมอ
  s.frozen = { ...s.frozen, ...cur.frozen };           // freeze ที่ tick เพิ่งเขียนต้องไม่หายไปกับ unfreeze ของบัญชีอื่น
  saveAp(s);
}
...
function apBroadcast(ev) {
  ...
  for (const res of apClients) {
    try { res.write(line); } catch { apClients.delete(res); }
  }
}
...
async function apGetCampaign(acct, token, s, acctId, d, objInfo, cf) {
  ...
  let mine = [];
  try {
    const all = await fbAll(`${acct}/campaigns`, { fields: 'id,name,status', limit: 200 }, token);
  ...
  const live = mine.find((c) => c.status === 'ACTIVE');
  if (live) {
    s.campaign = s.campaign || {};
    s.campaign[acctId] = live.id;
    s.owned = s.owned || [];
    if (!s.owned.includes(live.id)) s.owned.push(live.id);
    return live.id;
  }
...
async function apPauseLosers(cfg, prof, a, ads, s, alerts) {
  ...
  s.paused = s.paused || {};
  s.pausedLog = apRecent(s.pausedLog || [], 24 * 3600 * 1000);
  ...
  for (const ad of live) {
    ...
    try {
      ...
      // apRefill อ่านจากรายการเดียวกันนี้ที่ดึงมาตอนต้นรอบ — ไม่อัปเดตตรงนี้
      // มันจะยังนับแอดที่เพิ่งปิดว่ายังยิงอยู่ แล้วไม่เติมของใหม่จนกว่าจะถึงรอบหน้า
    ...
    for (const a of accts.filter((x) => x.account_status === 1)) {
      ...
      for (const ad of rejected) {
        ...
        const bad = nm.length < 10 ? 'ข้อความใหม่สั้นผิดปกติ'
          : nm.length > 5000 ? 'ข้อความใหม่ยาวผิดปกติ'
        ...
        try {
          const newId = await apResubmit(acct, prof.accessToken, ad.creative, ad.adset_id, ad.name, nm, dx.newHeadline);
...
async function runAutopilot(mode = 'full') {
  ...
  apRunning = true;
  try { await autopilotTick(mode); return true; }
...
app.post('/api/autopilot/run', async (req, res) => {
  ...
  try {
    if (!await runAutopilot()) return res.status(409).json({ error: 'กำลังตรวจอยู่ รอรอบนี้จบก่อน' });
...
app.get('/api/autopilot/stream', (req, res) => {
  ...
  const ping = setInterval(() => { try { res.write(': ping\n\n'); } catch { /* ปิดไปแล้ว */ } }, 25000);
  req.on('close', () => { clearInterval(ping); apClients.delete(res); });
});
