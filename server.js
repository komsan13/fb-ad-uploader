...
async function aiLandingCheck(cfg, apiKey) {
  const lp = loadLp();
  ...
  for (const l of (lp.links || []).filter((x) => /^https?:/i.test(x.url)).slice(0, 20)) {
    try {
      const r = await fetch(l.url, { redirect: 'follow', signal: AbortSignal.timeout(10000) });
      if (r.status >= 400) broken.push(`"${l.label}" (${l.url}) — HTTP ${r.status}`);
    } catch { /* ไม่ฟันธง */ }
  }
  let ai = null;
  try {
...
const TG_ACTION_TYPES = ['killSwitch', 'autopilotEnabled', 'testMode', 'setMinAds', 'setScaleMaxBudget', 'setLimit', 'setRuleCpr', 'unfreeze', 'campaignStatus', 'setCampaignBudget'];
...
- คำสั่งที่มีให้ใช้: killSwitch(on) หยุด/ปลดฉุกเฉิน • autopilotEnabled(on) • testMode(on) • setMinAds(value 0-50) • setScaleMaxBudget(value บาท, 0=ปิดขยายงบ) • setLimit(key, value) ปรับเพดาน • setRuleCpr(value) ตั้งเป้าต้นทุน/ผลลัพธ์ • unfreeze(accountId) • campaignStatus(campaignId, status ACTIVE|PAUSED, profileId) • setCampaignBudget(campaignId, accountId, profileId, value ต่อวัน)
...
function startBackgroundJobs() {
  ...
  saveWatchState(st);
    dailySummary().catch(() => {});
    weeklyAiJobs().catch(() => {});   // งานรายสัปดาห์เกาะรอบเช้าเดียวกัน มีธงวันที่กันซ้ำของตัวเอง
  }, 5 * 60 * 1000).unref();
}
...
if (require.main === module) {
  ...
  module.exports = { app, curFactor, apPrune, apMark, apRecent, apFence, resultSpec, pickResult, loadAp, saveAp, apLimits, apParseLimit, AP_LIMIT_SPEC, apSnapshot, saveApMerged, tgChunks, dailySummary, apRankCaptions, apCaptionPerf, aiCprReview, aiLandingCheck, aiWeeklyStrategy, aiTagVideos, weeklyAiJobs };
}
