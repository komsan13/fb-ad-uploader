...
test('apLimits: ตั้งเกินกรอบต้องถูกบีบกลับ ไม่ใช่ยอมตาม', () => {
  ...
  assert.strictEqual(wild.maxFixPerDay, 25);
...
test('กรอบของเกราะที่ห้ามถอดต้องไม่หย่อนเกิน 2.5 เท่าของค่าตั้งต้น', () => {
  ...
  for (const [k, s] of rails) {
    assert.ok(s.max <= s.def * 2.5, `${k}: max ${s.max} หย่อนเกิน 2.5 เท่าของค่าตั้งต้น ${s.def}`);
  }
});
