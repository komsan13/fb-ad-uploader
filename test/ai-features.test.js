...
describe('AI เขียนแคปชั่น (/api/captions/generate) — ต้องเป็นแบบร่างเสมอ', () => {
  ...
  test('ร่างที่ตรวจได้ medium ต้องให้ AI เขียนใหม่ และส่งเข้าคลังเฉพาะ low', async (t) => {
    const first = 'Ưu đãi hôm nay cho khách hàng mới, đăng ký ngay!';
    const replacement = 'Khám phá ưu đãi dành cho bạn hôm nay, xem chi tiết ngay!';
    const { base, ai } = await boot(t, { aiAnswer: [
      { variants: [{ message: first, headline: '' }] },
      { items: [{ index: 0, risk: 'medium', issues: [{ where: 'message', policy: 'Nội dung dễ gây hiểu nhầm', detail: 'Cần viết trung tính hơn' }], advice: 'Viết lại trung tính' }] },
      { variants: [{ message: replacement, headline: '' }] },
      { items: [{ index: 0, risk: 'low', issues: [], advice: null }] },
    ] });
    const r = await post(base, '/api/captions/generate', { count: 1 });
    assert.strictEqual(r.drafts.length, 1);
    assert.strictEqual(r.drafts[0].message, replacement, 'ห้ามส่งร่าง medium ตัวแรกเข้าคลัง');
    assert.strictEqual(r.drafts[0].risk, 'low');
    assert.strictEqual(ai.state.requests.length, 4, 'ต้องเขียน → ตรวจ → เขียนแก้ → ตรวจอีกครั้ง');
    assert.match(ai.state.requests[2].messages?.[0]?.content || '', /chưa qua kiểm tra chính sách/i, 'รอบแก้ต้องได้รับ feedback จากการตรวจนโยบาย');
    const all = await get(base, '/api/captions');
    assert.ok(!all.some((c) => c.message === first), 'ร่างเสี่ยงห้ามถูกบันทึก แม้เป็น draft');
  });
